...
notamment via la séparation des fonctions de logique et d'affichage.
  Les niveaux sont créés par des fichiers au format JSON, dont les
	éléments des cases initiaux sont simples à mettre en place via 
	un système de coordonnées au format [x, y] :
...
function checkCollision() {
  ...
  if (
		//Check si la tête du serpent va en dehors de la zone de jeu
    pro_x < 0 ||
    pro_y < 0 ||
    pro_x >= WORLD.length ||
    pro_y >= WORLD.length ||
		//Check si la tête du serpent recontre un mur ou son corps
...
function rafraichirScore(nouvScore) {
  document.getElementById("nbScore").textContent = nouvScore;
}
...
function dessinerVide(x, y) {
  ...
  context.fillRect(
    TAILLE_CASE * x + 1,
    TAILLE_CASE * y + 1, //Position du coin supérieur gauche
    TAILLE_CASE - 1,
    TAILLE_CASE - 1
  );
}
...
function dessinerQueue(x, y, pad) {
  ...
  context.fillRect(
    TAILLE_CASE * x + pad,
    TAILLE_CASE * y + pad, //Position du coin supérieur gauche
    TAILLE_CASE - pad * 2 + 1, //Largeur
    TAILLE_CASE - pad * 2 + 1  //Hauteur
  ); 
}

function dessinerTete(x, y, pad) {
  dessinerQueue(x, y, pad); 
  //Pour différencier la tête de la queue, le padding
...
function dessinerNourriture(x, y, pad) {
  context.beginPath();
  context.arc(
    TAILLE_CASE * (x + 0.5) + 0.5,
    TAILLE_CASE * (y + 0.5) + 0.5, //Position du centre
    TAILLE_CASE / 2 - pad, //Taille
    0,
    2 * Math.PI //Angle (2*PI = cercle entier)
  ); 
